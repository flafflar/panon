...
if arguments["--backend"] == "pyaudio":
    spectrum_source = source.PyaudioSource(
        spectrum.NUM_CHANNEL, sample_rate, arguments["--device-index"], cfg_fps
    )
elif arguments["--backend"] == "fifo":
    spectrum_source = source.FifoSource(
        spectrum.NUM_CHANNEL, sample_rate, arguments["--fifo-path"], cfg_fps
    )
...
#    spectrum_source = source.SounddeviceSource(spectrum.NUM_CHANNEL, sample_rate, arguments['--device-index'])
    ...
    spectrum_source = source.SoundCardSource(
        spectrum.NUM_CHANNEL, sample_rate, arguments["--device-index"], cfg_fps
    )
    from . import beat
...
async def mainloop():
    async with websockets.connect(arguments["<url>"]) as websocket:
        ...
        while True:
            smart_mode = (
                type(spectrum_source) is source.SoundCardSource
            ...
            if not use_glDFT and spectrum_data is None:
                ...
                isBeat = beatsDetector is not None and beatsDetector.isBeat(
                    latest_wave_data
                )
            ...
            data = spec.computeSpectrum(
                wave_hist,
                bassResolutionLevel=bassResolutionLevel,
